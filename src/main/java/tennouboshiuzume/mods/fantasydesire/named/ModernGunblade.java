...
public class ModernGunblade {
    ...
    public void init(LoadEvent.InitEvent event){
        ...
        ItemSlashBlade.SummonedSwordColor.set(tag, -0x010101);
        SpecialEffects.addEffect(customblade,FdSEs.TripleBullet);
        NBTTagCompound displayTag = new NBTTagCompound();
        customblade.setTagInfo("display",displayTag);
        NBTTagList loreList = new NBTTagList();
        loreList.appendTag(new NBTTagString(String.format(I18n.format("tennouboshiuzume.slashblade.MordernGunblade.desc"))));
        loreList.appendTag(new NBTTagString(String.format(I18n.format("tennouboshiuzume.slashblade.MordernGunblade.desc1"))));
        loreList.appendTag(new NBTTagString(String.format(I18n.format("tennouboshiuzume.slashblade.MordernGunblade.desc2"))));
...
//      是否为爆炸造成伤害
        if(!(event.damageSource.getTrueSource() instanceof EntityTNTPrimed)) return;
